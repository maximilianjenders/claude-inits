import { gh } from "../lib/exec.js";

const definition = {
  name: "gh_update_pr",
  description: "Update an existing pull request (title, body, milestone, labels, reviewers, base branch)",
  inputSchema: {
    type: "object",
    properties: {
      cwd: {
        type: "string",
        description: "Working directory (defaults to MCP server cwd)",
      },
      pr: {
        type: "number",
        description: "PR number to update",
      },
      title: {
        type: "string",
        description: "New title (optional)",
      },
      body: {
        type: "string",
        description: "New body content (optional)",
      },
      milestone: {
        type: "string",
        description: "Milestone title or number to set (optional)",
      },
      base: {
        type: "string",
        description: "New base branch (optional)",
      },
      add_labels: {
        type: "array",
        items: { type: "string" },
        description: "Labels to add (optional)",
      },
      remove_labels: {
        type: "array",
        items: { type: "string" },
        description: "Labels to remove (optional)",
      },
      add_assignees: {
        type: "array",
        items: { type: "string" },
        description: "Assignees to add (optional)",
      },
      remove_assignees: {
        type: "array",
        items: { type: "string" },
        description: "Assignees to remove (optional)",
      },
      add_reviewers: {
        type: "array",
        items: { type: "string" },
        description: "Reviewers to add (optional)",
      },
      remove_reviewers: {
        type: "array",
        items: { type: "string" },
        description: "Reviewers to remove (optional)",
      },
    },
    required: ["pr"],
  },
};

async function handler(args) {
  const {
    cwd,
    pr,
    title,
    body,
    milestone,
    base,
    add_labels,
    remove_labels,
    add_assignees,
    remove_assignees,
    add_reviewers,
    remove_reviewers,
  } = args;

  const opts = cwd ? { cwd } : {};
  const ghArgs = ["pr", "edit", String(pr)];

  if (title) {
    ghArgs.push("--title", title);
  }

  if (body) {
    ghArgs.push("--body", body);
  }

  if (milestone) {
    ghArgs.push("--milestone", milestone);
  }

  if (base) {
    ghArgs.push("--base", base);
  }

  if (add_labels && add_labels.length > 0) {
    for (const label of add_labels) {
      ghArgs.push("--add-label", label);
    }
  }

  if (remove_labels && remove_labels.length > 0) {
    for (const label of remove_labels) {
      ghArgs.push("--remove-label", label);
    }
  }

  if (add_assignees && add_assignees.length > 0) {
    for (const assignee of add_assignees) {
      ghArgs.push("--add-assignee", assignee);
    }
  }

  if (remove_assignees && remove_assignees.length > 0) {
    for (const assignee of remove_assignees) {
      ghArgs.push("--remove-assignee", assignee);
    }
  }

  if (add_reviewers && add_reviewers.length > 0) {
    for (const reviewer of add_reviewers) {
      ghArgs.push("--add-reviewer", reviewer);
    }
  }

  if (remove_reviewers && remove_reviewers.length > 0) {
    for (const reviewer of remove_reviewers) {
      ghArgs.push("--remove-reviewer", reviewer);
    }
  }

  // Check if any updates were requested
  if (ghArgs.length === 3) {
    return {
      content: [{ type: "text", text: "No updates specified" }],
    };
  }

  await gh(ghArgs, opts);

  // Fetch updated PR
  const { stdout } = await gh([
    "pr",
    "view",
    String(pr),
    "--json",
    "number,title,state,labels,milestone,assignees,reviewRequests,url",
  ], opts);

  const result = JSON.parse(stdout);

  return {
    content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
  };
}

export { definition, handler };
